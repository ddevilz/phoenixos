...
describe("getSimilarFailures", () => {
  it("extracts file paths from stack trace and returns predictions", async () => {
    ...
    const result = await getSimilarFailures(
      "Error in src/auth.py line 42\n  File src/db.py line 12"
    );
  ...
  it("deduplicates file paths extracted from the trace", async () => {
    ...
    await getSimilarFailures(
      "src/auth.py:1\nsrc/auth.py:2\nsrc/auth.py:3"
    );

    const body = JSON.parse(spy.mock.calls[0][1]?.body as string);
    expect(body.changed_files.filter((f: string) => f === "src/auth.py")).toHaveLength(1);
  });
});
