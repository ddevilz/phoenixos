...
async def test_fetch_and_parse_calls_github_apis() -> None:
    ...
    with (
        patch(
            "core.ingestor.diff_parser._fetch_diff",
            new_callable=AsyncMock,
            return_value=_SAMPLE_DIFF,
        ),
        patch(
            "core.ingestor.diff_parser._fetch_pr_head_sha",
            new_callable=AsyncMock,
            return_value="abc123",
        ),
        patch(
            "core.ingestor.diff_parser._fetch_file_contents",
            new_callable=AsyncMock,
            return_value="def test_validate(): pass",
        ),
    ):
...
async def test_evals_run_with_raw_diff() -> None:
    ...
    with (
        patch(
            "core.judge.behavior.BehaviorJudge.judge", new_callable=AsyncMock, return_value=_pass
        ),
        patch(
        ...
        patch(
            "core.judge.graph_writer.write_eval_result", new_callable=AsyncMock, return_value="eid"
        ),
