...
def _make_event(changed_files: list[str] | None = None) -> FailureEvent:
    return FailureEvent(
        id="evt-1", repo="owner/repo", run_id="99", workflow="CI",
        job="CI", step="unknown", exit_code=1, log_tail="error",
...
def _make_sig(embedding: list[float] | None = None) -> FailureSignature:
    return FailureSignature(
        id="sig-1", summary="test failure", category="test_failure",
...
async def test_pipeline_full_run_calls_write_and_embed() -> None:
    ...
    with (
        ...
        state = await pipeline.ainvoke({
            "event": event, "signature": None,
            "predictions": [], "at_risk": [], "fragility_scores": {},
        })
...
async def test_pipeline_state_has_predictions() -> None:
    ...
    expected_predictions = [
        {"id": "sig-1", "match_type": "direct", "confidence": 0.8,
         "summary": "auth error", "category": "build_error",
         "affected_component": "src/auth.py", "fragility_score": 0.8}
    ]
