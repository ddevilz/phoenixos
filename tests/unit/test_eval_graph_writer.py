...
def _make_aggregate(verdict: str = "pass", trust_score: float = 0.9) -> AggregateScore:
    return AggregateScore(
        ...
        judge_results=[
            JudgeResult(
                judge="behavior", score=trust_score, verdict=verdict, reasoning="ok", flags=[]
            ),
            JudgeResult(
                judge="security", score=trust_score, verdict=verdict, reasoning="ok", flags=[]
            ),
            JudgeResult(
                judge="regression", score=trust_score, verdict=verdict, reasoning="ok", flags=[]
            ),
        ],
    )
    ...
    aggregate = AggregateScore(
        ...
        judge_results=[
            JudgeResult(
                ...
                score=0.3,
                verdict="block",
                reasoning="broke contract",
                flags=["return type changed in login()"],
            ),
...
async def test_evals_run_endpoint_returns_aggregate_score() -> None:
    ...
    mock_result = JudgeResult(judge="behavior", score=0.9, verdict="pass", reasoning="ok", flags=[])

    with (
        patch(
            "core.judge.behavior.BehaviorJudge.judge",
            new_callable=AsyncMock,
            return_value=mock_result,
        ),
        patch(
            "core.judge.security.SecurityJudge.judge",
            new_callable=AsyncMock,
            return_value=JudgeResult(
                judge="security", score=0.9, verdict="pass", reasoning="ok", flags=[]
            ),
        ),
        patch(
            "core.judge.regression.RegressionJudge.judge",
            new_callable=AsyncMock,
            return_value=JudgeResult(
                judge="regression", score=0.9, verdict="pass", reasoning="ok", flags=[]
            ),
        ),
        patch(
            "core.judge.regression.fetch_similar_signatures",
            new_callable=AsyncMock,
            return_value=[],
        ),
        patch("core.db.neo4j.neo4j_session"),
        patch(
            "core.judge.graph_writer.write_eval_result",
            new_callable=AsyncMock,
            return_value="eval-id-1",
        ),
