...
async def test_genealogy_returns_chain_with_depth() -> None:
    ...
    mock_result.data = AsyncMock(return_value=[{
        "chain": [
            {"id": "fix-1", "description": "patch 1", "author_type": "human",
             "commit_sha": "abc1", "timestamp": "2026-01-01T00:00:00"},
            {"id": "fix-2", "description": "patch 2", "author_type": "ai",
...
async def test_genealogy_depth_zero_no_edges() -> None:
    ...
    mock_result = AsyncMock()
    mock_result.data = AsyncMock(return_value=[{
        "chain": [{"id": "fix-1", "description": "only fix", "author_type": "human",
                   "commit_sha": "abc1", "timestamp": "2026-01-01T00:00:00"}],
        "depth": 0,
    }])
    mock_session.run = AsyncMock(return_value=mock_result)
...
async def test_genealogy_warning_when_depth_exceeds_two() -> None:
    ...
    chain = [
        {"id": f"fix-{i}", "description": f"patch {i}", "author_type": "human",
         "commit_sha": f"abc{i}", "timestamp": "2026-01-01T00:00:00"}
        for i in range(1, 5)
    ]
    mock_session = AsyncMock()
    mock_result = AsyncMock()
